//___bb-20250708.c
//% gcc ___bb-20250708.c -o out -lm -O3
//% time ./out seed e_L e_Dr e_v0 e_Dth > _bb-20250708-data-'seed'-'e_L'-'e_Dr'-'e_v0'-'e_Dth'.txt &
//$ gcc -std=c99 ___bb-20250708.c -o out -lm -O3
//$ for a in {0..4};do for b in 10;do for c in 206 506 105 205 505 104 204;do for d in 204 504 103 203 503 102 202;do for e in 203;do bsub -q "jobLimit52" "./out $a $b $c $d $e > _bb-20250708-data-'$a'-'$b'-'$c'-'$d'-'$e'.txt";done;done;done;done;done
//$ gcc -std=c99 ___bb-20250708.c -o out -lm -O3
//$ for a in {0..4};do for b in 10;do for c in {0..8};do for d in {0..8};do for e in 203;do bsub -q "jobLimit52" "./out $a $b $c $d $e > _bb-20250708-data-'$a'-'$b'-'$c'-'$d'-'$e'.txt";done;done;done;done;done
...
//    double Dr=ConvertToParameter(strtol(argv[3],NULL,10)),v0=ConvertToParameter(strtol(argv[4],NULL,10)),Dth=ConvertToParameter(strtol(argv[5],NULL,10));
    double Dr=2.0*exp((-6.0+strtol(argv[3],NULL,10)/4.0)*log(10)),v0=2.0*exp((-4.0+strtod(argv[4],NULL)/4.0)*log(10)),Dth=ConvertToParameter(strtol(argv[5],NULL,10));
